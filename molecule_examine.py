...
def getSolubility(a):
    c = 10 ** a[0][0]
    print(c)
    b = "Расстворимость вещества: " + c.astype(str) + " моль/л"
    return b
...
smiles = st.text_input('⬇️ Введите структуру вещества или молекулы по правилу SMILES. '
                       'Например, как показона ниже, это химическое соединение "Тербутрин", который используется для '
                       '"Гербицида" -  вещества, применимый для уничтожения растений. Также ниже Вы увидете формулу и 3D модель',
...
st.write('Вещество считается нерастворимым, если его растворимость составляем менее 0.01 моль/л. Если растворимость составляет более 0.1 моль/л, то мы считаем его растворимым. Если его растворимость составляет от 0.01 до 0,1 моль/л, мы считаем, что оно малорастворимо.')
